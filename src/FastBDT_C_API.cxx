...
extern "C" {
  ...
  {
    reinterpret_cast<Expertise*>(ptr)->classifier.SetPurityTransformation(std::vector<bool>(purityTransformation,
        purityTransformation + size));
  }
  ...
  void* GetIndividualVariableRanking(void* ptr, float* array)
  {
    Expertise* expertise = reinterpret_cast<Expertise*>(ptr);
    VariableRanking* ranking = new (std::nothrow) VariableRanking;
    ranking->ranking = expertise->classifier.GetIndividualVariableRanking(std::vector<float>(array,
                       array + expertise->classifier.GetNFeatures()));
    return ranking;
  }
