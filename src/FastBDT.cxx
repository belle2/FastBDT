...
namespace FastBDT {
  ...
  EventValues::EventValues(unsigned int nEvents, unsigned int nFeatures, unsigned int nSpectators,
                           const std::vector<unsigned int>& nLevels) : values(nEvents * (nFeatures + nSpectators), 0), nFeatures(nFeatures),
    nSpectators(nSpectators)
  {

    if (nFeatures + nSpectators != nLevels.size()) {
      throw std::runtime_error("Number of features must be the same as the number of provided binning levels! " + std::to_string(
                                 nFeatures) + " + " + std::to_string(nSpectators) + " vs " + std::to_string(nLevels.size()));
    }
  ...
  void EventValues::Set(unsigned int iEvent, const std::vector<unsigned int>& features)
  {
    ...
    if (features.size() != nFeatures + nSpectators) {
      throw std::runtime_error(std::string("Promised number of features are not provided. ") + std::to_string(
  ...
  void EventSample::AddEvent(const std::vector<unsigned int>& features, Weight weight, bool isSignal)
  {
    ...
    if (nSignals + nBckgrds == nEvents) {
      throw std::runtime_error(std::string("Promised maximum number of events exceeded. ") + std::to_string(
                                 nSignals) + " + " + std::to_string(nBckgrds) + " vs " + std::to_string(nEvents));
    }
  ...
  std::vector<Weight> CumulativeDistributions::CalculateCDFs(const EventSample& sample, const unsigned int firstEvent,
  ...
  ForestBuilder::ForestBuilder(EventSample& sample, unsigned int nTrees, double shrinkage, double randRatio,
                               unsigned int nLayersPerTree, bool sPlot, double flatnessLoss) : shrinkage(shrinkage), flatnessLoss(flatnessLoss)
  {
  ...
  void ForestBuilder::updateEventWeightsWithFlatnessPenalty(EventSample& eventSample)
  {
    ...
    for (unsigned int iIndex = 0; iIndex < signal_event_index_sorted_by_F.size(); ++iIndex) {
      ...
      for (unsigned int iSpectator = 0; iSpectator < nSpectators; ++iSpectator) {
        ...
        double F_bin = weight_below_current_F_per_uniform_bin[iSpectator][uniformBin] / (uniform_bin_weight_signal[iSpectator][uniformBin] *
                       sums[0]);
    ...
    for (unsigned int iIndex = 0; iIndex < bckgrd_event_index_sorted_by_F.size(); ++iIndex) {
      ...
      for (unsigned int iSpectator = 0; iSpectator < nSpectators; ++iSpectator) {
        const uint64_t uniformBin = values.GetSpectator(iEvent, iSpectator);
        weight_below_current_F_per_uniform_bin[iSpectator][uniformBin] += weights.GetOriginalWeight(iEvent);
        double F_bin = weight_below_current_F_per_uniform_bin[iSpectator][uniformBin] / (uniform_bin_weight_bckgrd[iSpectator][uniformBin] *
                       sums[1]);
